...
from ratioComponent import ratioComponent as EMComponent
from geometryComponent import geometryComponent as driveComponent
...
class waveEnergy(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('Electromagnetics', EMComponent())
        self.add_subsystem('Drivetrain', driveComponent())
        self.add_subsystem('WavePower', DynamicsNewComponent())
        self.add_subsystem('Economics', structureComponent())
        self.add_subsystem('LifeCycleAssessment', environmentComponent())
        self.add_subsystem('-ilities', econComponent())
        self.add_subsystem('Output', outputComponent())
    ...
    def configure(self):
        ...
        for var_name in ivc_to_ratio:
            self.connect(f"ivc.{var_name}", f"EMComponent.{var_name}")

        #ivc to driveComponent

        # connect ivc to geo
        ivc_to_geo = ['D_f',   't_ft', 't_fr', 't_fc','t_fb', 't_sr', 't_dt', 'D_dt', 'theta_dt','M', 'rho_m', 'rho_w', 'm_scale']

        for var_name in ivc_to_geo:
            self.connect(f"ivc.{var_name}", f"driveComponent.{var_name}")

        # ratop tp driveComponent
        ratio_to_geo = ['D_s','T_f', 'h_f', 'h_s', 'D_d',  'T_s', 'h_d']
        for var_name in ratio_to_geo:
            self.connect(f"EMComponent.{var_name}", f"driveComponent.{var_name}")
        ...
        for var_name in ratio_to_dymn:
            self.connect(f"EMComponent.{var_name}", f"dynamicsComponent.{var_name}")

        #geo_to_dynam
        self.connect('driveComponent.m_f_tot', 'dynamicsComponent.m_float')
        self.connect('driveComponent.V_d', 'dynamicsComponent.V_d')
        self.connect('driveComponent.T', 'dynamicsComponent.draft')
        ...
        for var_name in ratio_to_struct:
            self.connect(f"EMComponent.{var_name}", f"structureComponent.{var_name}")

        # geo to structure
        self.connect('driveComponent.A_c', 'structureComponent.A_c')
        self.connect('driveComponent.A_lat_sub', 'structureComponent.A_lat_sub')
        self.connect('driveComponent.r_over_t', 'structureComponent.r_over_t')
        self.connect('driveComponent.I', 'structureComponent.I')
        ...
        #geo to environment
        self.connect('driveComponent.m_m', 'environmentComponent.steel')
        self.connect('driveComponent.A_fiberglass','environmentComponent.fiberglass')
        ...
        # geo to econ
        self.connect('driveComponent.m_m', 'econComponent.m_m')
        ...
        self.connect('driveComponent.V_f_pct', 'outcomeComponent.V_f_pct')
        self.connect('driveComponent.V_s_pct','outcomeComponent.V_s_pct')
        self.connect('driveComponent.GM', 'outcomeComponent.GM')
        ...
        self.connect('EMComponent.D_d','outcomeComponent.D_d')
