...
def inner_function(f_max, Vs_max, fb, wavefreq, amplitude):
...
Y = np.full((len_f_, len_Vs_), np.nan)
t = 1
for i in range(len_f_):
    for j in range(len_Vs_):
        Nfeval = 1
        try:
            Y[i,j] = inner_function(f_max = f_[i], Vs_max = Vs_[j], fb=RM3, wavefreq = 0.3, amplitude = 1)
...
fig, ax = plt.subplots()
im = ax.pcolormesh(Vs_, f_, Y, shading='nearest')
