...
class dynamicsComponent(om.ExplicitComponent):
    ...
    #confirmed
    def pick_which_root(self, roots, idx_no_sat, idx_zero, a_quad, b_quad, c_quad):
        which_soln = (roots == np.real(roots)) & (roots > 0) & (roots <= 1)
        both_ok = np.sum(which_soln, axis=2) == 2

        # temporarily mark the limit solutions
        # as having one solution, to ensure the
        # logic below works correctly
        which_soln[idx_no_sat | idx_zero, :] = True

        if np.any(both_ok):
            roots_equal = roots[:, :, 0] == roots[:, :, 1]
            if not np.all(roots_equal[both_ok]):
                print('Warning: Two valid non-equal solutions, choosing 1st arbitrarily')

            mult = self.handle_two_solns(both_ok, which_soln, roots,idx_no_sat, idx_zero, a_quad, b_quad, c_quad)
        else:
            num_solns = np.sum(which_soln, axis=2)
            if not np.all(num_solns == 1):
                # If there are some sea states with no valid solution,
                # proceed with which_soln set to False for them,
                # which will result in mult = 0 for those states
                print('Warning: Some sea states have no valid quadratic solution, so their energy is zeroed.')
    ...
    def get_relevant_soln(self, which_soln, roots, idx_no_sat,idx_zero):
    ...
    #confirmed
    def handle_two_solns(self, both_ok, which_soln, roots, idx_no_sat, idx_zero, a, b, c):
        row, col = np.where(both_ok)
        which_soln[row, col, 1] = False
        mult_1 = self.get_relevant_soln(which_soln, roots, idx_no_sat, idx_zero)
        mult = mult_1
