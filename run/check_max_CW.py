import numpy as np
import matplotlib.pyplot as plt

def check_max_CW():
    p = parameters()
    p['cost_m'] = [0, 0, 0]  # hack that makes cost constant, so minimizing LCOE is actually maximizing power
    p['power_max'] = np.inf
    b = var_bounds()
    x0 = b['X_start_struct']

    # run LCOE minimization (effectively power maximization)
    X_opt = gradient_optim(x0, p, b, 1)

    # plug back into simulation to get unsaturated power
    _, _, _, _, val = simulation(X_opt, p)
    P_unsat = val['power_unsat']

    # calculate capture width
    T, Hs = np.meshgrid(p['T'], p['Hs'])
    P_wave = p['rho_w'] * p['g']**2 / (64 * np.pi) * T * Hs**2
    CW = P_unsat / P_wave

    # compare to maximum capture width
    CW_max = p['g'] * T**2 / (4 * np.pi**2)

    ratio = CW / CW_max

    # plotting
    plt.contourf(T, Hs, ratio)
    plt.xlabel('Wave Period T (s)')
    plt.ylabel('Wave Height Hs (m)')
    plt.colorbar()
    plt.grid(True)
    plt.title('CW / CW_{max}')
    plt.show()

    return ratio
