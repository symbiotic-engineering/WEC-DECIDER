...
def inner_function(f_max, x_max, Vs_max, fb, wavefreq, amplitude):
    ...
    results = wec.solve(
        ...
        # x_wec_0=np.ones(wec.nstate_wec) * 0,
        # x_opt_0=np.ones(nstate_opt) * 0,
        callback=callbackF,
...
Vs_ = np.linspace(5e4, Vs_max, len_Vs_)
...
t = 0
X = np.full(len_f_*len_x_*len_Vs_, np.nan)
for i in range(len_f_):
    for j in range(len_x_):
        for k in range(len_Vs_):
            Nfeval = 1
            try:
                X[t] = inner_function(f_[i], x_[j], Vs_[k], fb=RM3, wavefreq = 0.3, amplitude = 1)
...
sc = ax.scatter(f, x, Vs, c=X, marker='o', s=25, depthshade=False)
